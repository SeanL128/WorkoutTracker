...
class StatsViewModel: ObservableObject {
    ...
    @Published var selectedGraphView: String = "1M"
    ...
    var titleView: some View {
        VStack {
            Text(selectedWorkout?.name ?? "Overall")
                .font(.title)
            Text("\(selectedMuscleGroupRepBreakdown[.overall] ?? 0) reps")
                .font(.callout)
            Text("\(selectedMuscleGroupWeightBreakdown[.overall]?.formatted() ?? 0.formatted())lbs")
                .font(.callout)
            Text("\(lengthToString(length: selectedTotalTime))")
                .font(.callout)
        }
    }
}
